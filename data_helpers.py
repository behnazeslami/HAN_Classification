...
def create_prediction_file(output_file, all_labels, all_predict_labels, all_predict_values):
    """
...
def data_word2vec(input_file, num_labels, word2vec_model):
    ...
    with open(input_file) as fin:
        ...
        for eachline in fin:
            ...
            label_index = data['label'].strip().split()
            ...
            labels_num.append(data['label'])
            content_indexlist.append(token_to_index(content))

            if 'label' in data.keys():
                labels_bind.append(data['label'])
