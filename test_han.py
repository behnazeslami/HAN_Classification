...
def test_han():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            print("☛ All Test Dataset: Loss {0:g}".format(test_loss))

            # Predict by threshold
            print("︎☛ Predict by threshold: Recall {0:g}, accuracy {1:g}, F {2:g}"
                        .format(test_rec, test_acc, test_F))
            # Save the prediction result
            if not os.path.exists(SAVE_DIR):
                os.makedirs(SAVE_DIR)
            dh.create_prediction_file(output_file=SAVE_DIR + '/predictions.json',
